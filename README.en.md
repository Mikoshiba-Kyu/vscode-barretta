...
This extension supports automatic language switching based on VSCode's **Display Language** setting. If the current language is not available, English content will be displayed by default.

We welcome contributions for localization. Localization instructions are as follows:

- `LOCALE_ID` = VSCode-defined locale code, reference: ([VSCode-locales documentation](https://code.visualstudio.com/docs/configure/locales#_available-locales))\
  Examples: `en`, `zh-cn`, `ja` represent English (United States), Simplified Chinese, and Japanese
- All localization configuration files are stored in the `/src/l10n` path
    - Plugin status output popups are stored in: `bundle.l10n.<LOCALE_ID>.json`
    - Plugin sidebar window-related content is stored in: `sidebar-menu/<LOCALE_ID>.json`
    - VSCode settings, menu commands, and output-related content are stored in: `package.l10n.<LOCALE_ID>.json`
- The default language is English.
- Only for sidebar-menu localization files, the English file needs to specify `<LOCALE_ID>`.\
  For package and bundle localization files, the English file does not need to specify `<LOCALE_ID>`